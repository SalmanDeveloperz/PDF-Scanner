...
const toolGroups = [
  ...
  },
  {
    ...
    icon: FileOutput,
    tools: [
      { icon: FileImage, title: "Image to PDF", description: "Turn photos into a polished multi-page PDF.", href: "/image-to-pdf" },
      { icon: FileOutput, title: "PDF to JPG", description: "Export pages as image files." },
      { icon: FileInput, title: "Word to PDF", description: "Create a PDF from DOC and DOCX files." },
      { icon: FileInput, title: "Excel to PDF", description: "Create a PDF from spreadsheet files." },
      { icon: FileInput, title: "PowerPoint to PDF", description: "Create a PDF from slide presentations." },
    ],
  },
  {
    id: "edit-pdf",
    ...
    icon: Pencil,
    tools: [
      { icon: PenLine, title: "Sign PDF", description: "Place a drawn signature on your pages.", href: "/sign-pdf" },
      { icon: Stamp, title: "Watermark PDF", description: "Add a text label to your document pages.", href: "/watermark-pdf" },
      { icon: Pencil, title: "Edit PDF", description: "Add text, notes, and shapes to a document." },
  ...
  },
  {
    ...
    icon: ShieldCheck,
    tools: [
      { icon: LockKeyhole, title: "Lock PDF", description: "Protect a document with a password.", href: "/lock-pdf" },
  ...
  return (
    <div id="top" className="min-h-screen bg-background">
      ...
      <main>
          ...
          <div className="mx-auto max-w-7xl px-5 py-10 sm:py-12 lg:px-8 lg:py-14">
            <div className="catalog-heading">
              <div className="catalog-title-block">
                <span className="catalog-kicker"><span className="catalog-kicker-dot" /> YOUR DOCUMENT WORKSPACE</span>
                <h1 id="tools-heading" className="mt-3 text-3xl font-black tracking-tight sm:text-4xl lg:text-5xl">Your essential tools, all in one place</h1>
                <p className="catalog-tagline">Make every document <span className="tools-typewriter" aria-label={`Make every document ${headline || rotatingHeadlines[0]}`}>{headline}<span className="typewriter-caret" aria-hidden="true" /></span></p>
              </div>
              <div className="catalog-intro">
                <p>One calm workspace for scanning, organizing, converting, and securing documents.</p>
                <div className="catalog-trust"><span><ShieldCheck /> Files stay on your device</span><span><Check /> No account needed</span></div>
              </div>
            </div>
            ...
            <div className="catalog-controls">
              <label className="tool-search">
                ...
                />
                {toolSearch && <button type="button" onClick={() => setToolSearch("")} aria-label="Clear search">Clear</button>}
              </label>
              <div className="catalog-summary"><strong>{matchingTools.length}</strong> tools <span aria-hidden="true">·</span> <strong>{allTools.filter((tool) => tool.href).length}</strong> available now</div>
            </div>
            ...
            <div className="catalog-groups">
              {toolGroups.map(({ id, title, description, icon: GroupIcon, tools }) => {
                const visibleTools = tools.filter((tool) => matchingTools.some((match) => match.title === tool.title));
                if (!visibleTools.length) return null;
                return (
                  <section key={id} id={id} className="catalog-group scroll-mt-32" aria-labelledby={`${id}-heading`}>
                    <div className="catalog-group-heading">
                      <span className="catalog-group-icon"><GroupIcon aria-hidden="true" /></span>
                      <div><h2 id={`${id}-heading`}>{title}</h2><p>{description}</p></div>
                      <span className="catalog-group-count">{visibleTools.length} {visibleTools.length === 1 ? "tool" : "tools"}</span>
                    </div>
                    <div className="catalog-tool-grid">
                      {visibleTools.map(({ icon: Icon, title: toolTitle, description: toolDescription, href }) => (
                        href ? (
                          <a key={toolTitle} href={href} className="catalog-tool-card catalog-tool-available">
                            <span className="catalog-tool-icon"><Icon aria-hidden="true" /></span>
                            <span className="catalog-tool-copy"><strong>{toolTitle}</strong><small>{toolDescription}</small></span>
                            <span className="catalog-tool-status catalog-status-ready">Available <ArrowRight aria-hidden="true" /></span>
                          </a>
                        ) : (
                          <article key={toolTitle} className="catalog-tool-card catalog-tool-upcoming" aria-label={`${toolTitle}, coming soon`}>
                            <span className="catalog-tool-icon"><Icon aria-hidden="true" /></span>
                            <span className="catalog-tool-copy"><strong>{toolTitle}</strong><small>{toolDescription}</small></span>
              ...
              {matchingTools.length === 0 && (
                <div className="catalog-empty-state"><Search aria-hidden="true" /><h2>No tools found</h2><p>Try another search, such as “merge”, “image”, or “security”.</p><button type="button" onClick={() => setToolSearch("")}>Show all tools</button></div>
              )}
            </div>
          </div>
        </section>
        ...
        <section id="how-it-works" className="reveal scroll-mt-20 bg-soft py-14 sm:py-20" data-reveal>
          <div className="mx-auto max-w-7xl px-5 lg:px-8">
